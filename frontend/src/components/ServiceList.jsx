...
import { Search, MapPin, IndianRupee, Filter, Star, Loader2 } from 'lucide-react';
...
export default function ServiceList() {
  ...
  const categories = [
    "PHOTOGRAPHY", "CATERING", "VENUE", "DECORATION", "MUSIC", "MAKEUP", "TRANSPORT", "OTHER"
  ];
        ...
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="bg-white/70 backdrop-blur-xl p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white relative overflow-hidden">
            ...
            <div className="text-center mb-8">
              ...
              <p className="mt-3 text-lg text-slate-600 font-medium">Find the perfect vendors and services for your next event.</p>
            </div>
              ...
              <AnimatePresence>
                {filteredServices.length > 0 ? filteredServices.map((service, index) => (
                  <motion.div
                    ...
                    className="bg-white/80 backdrop-blur-md border border-white rounded-3xl p-5 shadow-[0_8px_20px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all group flex flex-col h-full"
                  >
                      ...
                      {service.photos && service.photos.length > 0 ? (
                        <div className="relative w-full h-full group-hover:scale-105 transition-transform duration-500">
                          {service.photos.slice(0, 3).map((photo, i) => {
                            const isTop = i === 0;
                            const rotation = isTop ? 0 : (i % 2 === 0 ? 6 : -6) * i;
                            return (
                              <img 
                                key={i}
                                src={photo}
                        ...
                        <div className="w-full h-full bg-indigo-50 rounded-2xl border-4 border-white shadow-sm flex items-center justify-center">
                          <span className="text-indigo-200 font-medium">No photos</span>
                        </div>
                      )}

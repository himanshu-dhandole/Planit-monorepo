...
import { MapPin, Star, ShieldCheck, Clock, ShieldAlert, Loader2 } from 'lucide-react';
...
export default function VendorList() {
  ...
  useEffect(() => {
    const fetchVendors = async () => {
      try {
        const response = await apiClient.get('/api/public/vendor?page=0&size=50');
        // The API returns a Spring Page object, so vendors are in response.data.content
                ...
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-50 overflow-hidden flex-shrink-0 border-4 border-white shadow-sm flex items-center justify-center text-indigo-300 font-bold text-2xl">
                    {vendor.profileImageUrl ? (
                      <img src={vendor.profileImageUrl} alt={vendor.businessName} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0 pt-1">
                    ...
                    <div className="flex items-center gap-1.5 text-sm text-slate-500 mt-1 font-medium">
                      ...
                      <span className="truncate">{vendor.addressLine1 || 'Location varies'}, {vendor.state}</span>
                    </div>
                  </div>
                </div>
